// From: https://github.com/jojotastic777/bitburner-declarations/blob/master/src/BitBurner.d.ts
...
declare interface IGame {
  ...
  wget(url: string, file: string): Promise<boolean>;
  ps(
    hostname?: string
  ): Array<{ filename: string; threads: number; args: Array<string> }>;
  sprintf(rg0format: string, ...args: any): string;
  getPurchasedServerCost(desiredRam: number): number;
  getPurchasedServerLimit(): number;
  hackAnalyzePercent(hostname: string): number;
  hackChance(hostname: string): number;
  hacknetnodes: IHacknetNode[];

  hack(hostname: string, { threads: number } = {}): Promise<number>;
  grow(hostname: string, { threads: number } = {}): Promise<number>;
  weaken(hostname: string, { threads: number } = {}): Promise<number>;
